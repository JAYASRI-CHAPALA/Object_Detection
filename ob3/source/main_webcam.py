...
while True:
    ...
    confs = list(np.array(confs).reshape(1, -1)[0])  # Flatten the confidence array
    confs = list(map(float, confs))
    ...
    # Check if any bounding boxes are returned by NMS
    if len(indicies) > 0:
        # Ensure indicies is a list of lists (2D array)
        for i in indicies.flatten():
            box = bbox[i]
            x, y, w, h = box[0], box[1], box[2], box[3]
            cv2.rectangle(image, (x, y), (x + w, y + h), color=(0, 255, 0), thickness=2)
            
            # Directly access classIds[i] instead of classIds[i][0]
            cv2.putText(image, classNames[classIds[i] - 1], (box[0] + 10, box[1] + 30),
                        cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
    else:
        print("No objects detected or valid bounding boxes.")
